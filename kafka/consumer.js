		...
		eachMessage: async ({ topic, partition, message }) => {
			...
			if (msg) {
				...
				if (msg.estado === "CREDITO_FAIL") {
					...
					console.log(
						"Falló el pago. Cambia estado de la orden a fallida"
					);
					try {
						await orden.cambiaStatus("ORDEN_FAIL");
						console.log(
							"Envía notificación correspondiente a orden no generada"
						);
					} catch (error) {
						console.log(error);
					}
					...
					console.log(
						"Pago exitoso. Cambia estado de la orden a exitosa"
					);
					try {
						await orden.cambiaStatus("ORDEN_OK");
						console.log(
							"Envía notificación correspondiente a orden generada"
						);
					} catch (error) {
						console.log(error);
					}
...
consume().catch((e) =>
	console.error(`Error al consumir notificación ${e.message}`, e)
);
